module.exports = (input) => {
  ...
  symbolTable.bootstrap();
  semanticAnalyzer.bootstrap();
  ...
  let isVectorIndexHappening = false;
  // UTIL VARIABLES - END
  ...
  function Start() {
    Program();
    return {syntaxErrors: errorLog, semanticErrors: semanticAnalyzer.get()};
  }
  ...
  function BaseValue() {
    if (currentClass() == STRING_LITERAL || currentToken() == "vazio" || currentToken() == "verdadeiro" || currentToken() == "falso") {
      if (isAssignmentHappening) {
        ...
        if (errorType) {
          const error = `Linha: ${currentLine()}: Atribuição inválida. Tipo de símbolo de origem [${currentToken()}][${errorType}] diferente do de destino ${currentAssignmentId}[${currentAssignmentType}].`;
  ...
  function NumberLiteral() {
    if (currentClass() == NUMBER) {
      ...
      if (isVectorIndexHappening) {
        if (!/^\-?\s*[0-9]+$/.test(currentToken())) {
          const error = `Linha: ${currentLine()}: Indice invalido para vetor. Esperava um valor inteiro e veio [${currentToken()}][${errorType}].`;
          semanticAnalyzer.insert(error);
        }
      }
      nextToken();
  ...
  function VectorIndex() {
    if (currentToken() == "[") {
      nextToken();
      isVectorIndexHappening = true;
      Expression();
      isVectorIndexHappening = false;
      if (currentToken() == "]") {
        nextToken();
        VectorIndex();
        return;
  ...
  function Assignment() {
    ...
    if (!existsGlobalScope && !existsLocalScope) {
      ...
      currentAssignmentId = currentAssignmentId['id'];
    }
